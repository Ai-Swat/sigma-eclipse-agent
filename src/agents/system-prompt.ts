...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    availableTools.has("browser") && availableTools.has("web_fetch")
      ? "Browser vs web_fetch: if the user asks to open a tab, open/visit a site, navigate, go to, browse, or interact with a page (including RU triggers: 'открой', 'открой вкладку', 'открой сайт', 'зайди на', 'перейди на', 'посмотри', 'кликни', 'введи'), ALWAYS use the `browser` tool (action=open / navigate / snapshot / act — the action for opening a new tab is literally \"open\", NOT \"open_tab\"). Use `web_fetch` only for pure, non-interactive text extraction when no browser UI is needed."
      : "",
